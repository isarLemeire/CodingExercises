...
int main(int argc, char* argv []) {
	...
	// Read ADFGX settings
	char* settings = read_from_file("data/settings.txt");

	// Extract substrings
	char* alphabet = strdup(strtok(settings, "\r\n"));
	char* symbols = strdup(strtok(NULL, "\r\n"));
	char* dictionary = strdup(strtok(NULL, "\r\n"));
	char* transposition = strdup(strtok(NULL, "\r\n"));
	free(settings);
	printf(alphabet);
	...
	// Print dictionary
	printf("\nDictionary:\n\n");
	for (int i = 0, l = strlen(m->symbols); i < l; i++) {
		for (int j = 0; j < l; j++) {
			int k = i * l + j;
	...
	// Read plain text
	char* text = read_from_file("data/text.txt");
	printf("\nPlain text message:\n\n   %s\n\n", text);
	...
	char* input = read_from_file("data/encoded.txt");
	...
	if (decoded_message) {
		...
		printf("   %s\n\n", decoded_message);
		write_to_file("data/decoded.txt", decoded_message);
